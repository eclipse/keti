...
class NonHierarchicalResourcesIT : AbstractTestNGSpringContextTests() {
    ...
    @Test
    @Throws(Exception::class)
    fun testResourceWithParentsFailWhenNotUsingGraph() {
        val resource = JSON_UTILS.deserializeFromFile(
            "controller-test/a-resource-with-parents.json", BaseResource::class.java
        )
        ...
        val putContext = TEST_UTILS.createWACWithCustomPUTRequestBuilder(
            this.wac,
            TEST_ZONE.subdomain,
            "$RESOURCE_BASE_URL/%2Fservices%2Fsecured-api-with-parents"
        )
