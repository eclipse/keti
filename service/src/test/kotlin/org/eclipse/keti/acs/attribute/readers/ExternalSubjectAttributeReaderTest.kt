...
class ExternalSubjectAttributeReaderTest {
    ...
    internal fun beforeMethod() {
        ...
        this.externalSubjectAttributeReader = ExternalSubjectAttributeReader(null, this.attributeCache, 3000)
    ...
    fun testGetAttributesWithCacheMiss() {
        ...
        Mockito.doReturn(this.expectedAdapterAttributes)
            .`when`<ExternalSubjectAttributeReader>(this.externalSubjectAttributeReader)
            .getAttributesFromAdapters(IDENTIFIER)

        val actualAdapterAttributes = this.externalSubjectAttributeReader.getAttributes(IDENTIFIER)
    ...
    fun testGetAttributesWithCacheHit() {
        ...
        Mockito.verify<ExternalSubjectAttributeReader>(this.externalSubjectAttributeReader, Mockito.times(0))
            .getAttributesFromAdapters(IDENTIFIER)
    ...
    fun testGetAttributesByScope() {
        ...
        var actualAttributes = this.externalSubjectAttributeReader.getAttributes(IDENTIFIER)

        Assert.assertEquals(this.expectedAdapterAttributes!!.attributes, actualAttributes)

        actualAttributes = this.externalSubjectAttributeReader.getAttributesByScope(
            IDENTIFIER,
            setOf(Attribute("test-issuer", "test-scope", "test-value"))
        )
    ...
    fun testGetAttributesThatAreToLarge(
        ...
        ExternalAttributeReaderHelper.setupMockedAdapterResponse(
            this.externalSubjectAttributeReader, this
                .attributeCache, IDENTIFIER
        )
        ReflectionTestUtils
            .setField(this.externalSubjectAttributeReader, "maxNumberOfAttributes", maxNumberOfAttributes)
        ReflectionTestUtils.setField(
            this.externalSubjectAttributeReader, "maxSizeOfAttributesInBytes",
