...
class ResourceHttpMethodsFilterTest {

    @InjectMocks
    private lateinit var resourcePrivilegeManagementController: ResourcePrivilegeManagementController
    ...
    fun setup() {
        ...
        this.mockMvc = MockMvcBuilders.standaloneSetup(this.resourcePrivilegeManagementController)
            .addFilters<StandaloneMockMvcBuilder>(ResourceHttpMethodsFilter()).build()
    }
    ...
    fun urisAndTheirAllowedHttpMethods(): Array<Array<Any?>> {
        return arrayOf<Array<Any?>>(
            ...
            arrayOf(
                "/v1/resource", HashSet(
                    listOf(HttpMethod.POST, HttpMethod.GET, HttpMethod.HEAD, HttpMethod.OPTIONS)
                )
            )
        )
    }
