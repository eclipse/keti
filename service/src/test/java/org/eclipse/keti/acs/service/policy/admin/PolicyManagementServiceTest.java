...
public class PolicyManagementServiceTest extends AbstractTransactionalTestNGSpringContextTests {
    ...
    @Test
    public void testCreatePolicySetWithObligations() {
        PolicySet policySet = this.jsonUtils.deserializeFromFile("obligation/set-with-obligation-expressions.json",
                PolicySet.class);
        String policyName = policySet.getName();
        this.policyService.upsertPolicySet(policySet);
        PolicySet savedPolicySet = this.policyService.getPolicySet(policyName);
        Assert.assertNotNull(savedPolicySet);
        Assert.assertEquals(this.jsonUtils.readJsonNodeFromObject(policySet).toString(),
                this.jsonUtils.readJsonNodeFromObject(savedPolicySet).toString());
        this.policyService.deletePolicySet(policyName);
        Assert.assertEquals(this.policyService.getAllPolicySets().size(), 0);
    }

    @Test(expectedExceptions = { PolicyManagementException.class })
    public void testCreatePolicySetWithInvalidObligations() {
        PolicySet policySet = this.jsonUtils.deserializeFromFile("obligation/obligation-expression-id-not-unique.json",
                PolicySet.class);
        this.policyService.upsertPolicySet(policySet);
    }
