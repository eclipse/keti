...
class MonitoringHttpMethodsFilterTest {

    @InjectMocks
    private lateinit var acsMonitoringController: AcsMonitoringController
    ...
    fun setup() {
        ...
        this.mockMvc = MockMvcBuilders.standaloneSetup(this.acsMonitoringController)
            .addFilters<StandaloneMockMvcBuilder>(MonitoringHttpMethodsFilter()).build()
    }
    ...
    fun urisAndTheirAllowedHttpMethods(): Array<Array<Any?>> {
        return arrayOf(
            arrayOf<Any?>(
                ...
                HashSet(Arrays.asList(HttpMethod.GET, HttpMethod.HEAD, HttpMethod.OPTIONS))
            )
        )
    }
}
